...
const { createPermission, updatePermission } = require('../validators/permissionSchemas');
...
exports.get = asyncHandler(async (req, res) => {
  ...
  if (!row) return res.status(404).json({ message: 'Permission not found' });
  res.json(row);
});
...
exports.update = [
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!exists) return res.status(404).json({ message: 'Permission not found' });
